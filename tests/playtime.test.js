...
module.exports = {
  'バーは のこり じかんで みじかく なり、せっていで じかんを かえられる（なし なら 出ない）': async t => {
    ...
    t.eq(await p.evaluate(() => play.limit), 20, 'はじめは 20ぷん');
    await p.evaluate(MIN => { play._set(5*MIN); play.update(); }, MIN);
    let s = await state(p);
  ...
  'のこり 3ぷん・1ぷんで「あと ○ぷん」を てんめつ': async t => {
    const p = await t.open();
    await p.evaluate(MIN => { play._set(17*MIN - 500); play.update(); }, MIN);
  ...
  '0 に なっても といている もんだいが おわるまで まつ。「もうすこし」で とじて つづけられる': async t => {
    const p = await t.open();
    await p.evaluate(MIN => { taskOn = true; startTask(0); play._set(20*MIN + 1000); play.update(); }, MIN);
  ...
  'ひっさんを といている あいだは まつ。1じかん やすんだら いっぱいに もどる': async t => {
    const p = await t.open();
    await p.evaluate(MIN => { taskOn = true; startTask(0); taskDone = true; startHissanTask(); play._set(21*MIN); play.update(); }, MIN);
